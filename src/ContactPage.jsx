...
function ContactPage(props) {
  const renderInfo = (img, title, info) => {
    return (
      <div className=" flex gap-4">
        ...
        <div className="">
          <p className="text-blue-500 font-bold text-[clamp(1rem,5vw,25px)] md:text-[clamp(1rem,1.5vw,50px)] select-none">
            {title}
          </p>
          <p
            className={`md:text-[clamp(1rem,1vw,35px)] text-[clamp(1rem,5.5vw,40px)]  ${
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-blue-500 font-bold text-[clamp(1.2rem,5vw,25px)] md:text-[clamp(1rem,1.8vw,50px)] select-none">
          Contact
        </p>
        <p
          className={`${
            props.background ? 'text-white' : 'text-black'
          } font-bold text-[clamp(1rem,5.5vw,26px)] md:text-[clamp(1rem,2.5vw,60px)] select-none`}
        >
          I'd be happy to hear from you. 👇
        </p>

        <div className=" flex p-2 gap-7 flex-col items-center md:p-0 md:items-start md:flex-row my-5 md:my-10 ">
